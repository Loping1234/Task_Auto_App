...
import './TeamManagement.css'; // Reusing existing styles
...
const EditTeam = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [teamRes, subadminsRes, employeesRes] = await Promise.all([
                teamsAPI.getById(decodedName),
                subadminsAPI.getAll(),
                employeesAPI.getAll()
            ]);

            const team = teamRes.data.teams?.[0] || teamRes.data.team; // API might return array or single obj depending on implementation

            // If we used getAll() logic to find by name, we might need to filter. 
            // But let's check api.js: app.get("/api/teams", ...) returns all.
            // Wait, api.js has app.get("/api/teams/:teamName"...) ? 
            // No, src/api.js ONLY has app.get("/api/teams"). It DOES NOT have getById specific endpoint?
            // Let me re-read src/api.js in step 227.
            // Line 421: app.get("/api/teams"...)
            // It DOES NOT have get single team endpoint!
            // I need to fix the backend API to support getting a single team first!

            setSubadmins(subadminsRes.data.subadmins || []);
            setEmployees(employeesRes.data.employees || []);

            // Temporary workaround until I check API again: fetch all and find
            const allTeams = await teamsAPI.getAll();
            const foundTeam = allTeams.data.teams.find(t => t.teamName === decodedName);

            if (foundTeam) {
                setOriginalTeam(foundTeam);
                setFormData({
                    newTeamName: foundTeam.teamName,
                    subadminEmail: foundTeam.subadminEmail,
                    employees: foundTeam.employees || []
                });
            } else {
